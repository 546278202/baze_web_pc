...
export default {
  ...
  methods: {
	  checkBindPhone() {
			...
			this.axios.get(this.Global.BASE_URL + `/index.php?r=web/site/wxcallbake`,{params:params}).then(response => {
				if(response.code=='0'){
					// 登录成功
    ...
    commitData() {
      this.$refs.loginForm.validate(valid => {
        ...
        if (valid) {
		  ...
		  console.log(params);
          this.axios.get(this.Global.BASE_URL + "/index.php?r=web/site/banding", { params: params}).then(response => {
			  	console.log(response);
              	if (response.code == "0") {
                	this.$store.commit("changeLogin",JSON.stringify(response.data));
