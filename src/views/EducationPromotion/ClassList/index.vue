        ...
        <div class="item">
            ...
            <ol style="float:left;overflow:hidden;">
                <li v-for="(item,index) in dataList" :key="index" @click="tabClick(item)">
                    <div class="video-top">
                        <img style="width: 150px; height: 150px" :src="Global.img_BASE_URL+item.sSchonLogo" fit="fit"  @mouseover="enter($event)" @mouseleave="leave($event)" />
                    </div>
                    <div class="video-bottom">
                        <div class="describe"> {{item.sSchooName}} </div>
                        <div class="describe-data">
                            <div class="left">累计报名{{item.sEnroll}}</div>
                            <div class="right" style="float:right;">学制2.5年</div>
                        </div>
                        <div class="tag-item">
...
filters: {
  ...
  methods: {
    ...
    tabClick(item) {
        this.$router.push({ path: `/collegeDetail?id=${item.sid}`});
    },
    moreHandle() {
        this.$router.push({ path: `/collegesList`});
    },
